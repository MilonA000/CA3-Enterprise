      ...
      <main className={styles.timetablePageLayout}>
        ...
        <div className={styles.timetableContentColumn}>
          <section className={`${styles.section} ${styles.timetablePanel}`}>
            ...
            {authLoading && (
              <div className={styles.messageCard}>Checking login status...</div>
            )}

            {!authLoading && !user && (
              <div className={styles.messageCard}>Please login to view timetable information.</div>
            )}
                ...
                <div className={styles.dayTabs} role="tablist" aria-label="Select timetable day">
                  ...
                  {DAYS.map((day) => (
                    <button key={day} type="button" role="tab" aria-selected={selectedDay === day} className={`${styles.dayButton} 
                        ${selectedDay === day ? styles.activeDayButton : ""}`} onClick={() => setSelectedDay(day)}>{day}</button>
                  ))}
                </div>
                    ...
                    <div className={styles.tableWrapper}>
                      <table className={styles.table}>
                        ...
                        <tbody>
                          {entries.map((entry) => (
                            <tr key={entry.id}>
                              <td className={styles.timeCell}>{formatTime(entry.start_time)} -{" "}{formatTime(entry.end_time)}</td>
                              ...
                              <td><span className={styles.modeBadge}>{entry.delivery_mode}</span>
                              </td>
