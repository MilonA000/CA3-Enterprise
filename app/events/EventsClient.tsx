...
export default function EventsClient({
  ...
  const filteredEvents = useMemo(() => {
    return events.filter((event) => {
      ...
      const matchesSociety = !societySlug || event.societySlug === societySlug;
      ...
      {modelSummary && (
        <>
          <section className={styles.resultsCount} aria-label="Attendance model summary">
            <strong>Expected attendance model:</strong>{" "}
            {modelSummary.modelChoiceExplanation} Features:{" "}
            {modelSummary.featuresUsed.join(", ")}. Evaluation on fictional
            history (train/test split {modelSummary.trainSize}/
            {modelSummary.testSize}): MAE {modelSummary.meanAbsoluteError}, band
            accuracy {modelSummary.testBandAccuracy}%.
          </section>

          <section className={styles.resultsCount} aria-label="Data generation details"><strong>Data:</strong> {modelSummary.generatedDataDescription}</section>
        </>
      )}
              ...
              <div className={styles.tags}>
                ...
                {event.predictedBand && (
                  <span className={styles.tag}>
                    Expected attendance: {event.predictedBand} {event.predictedAttendance ? ` (${event.predictedAttendance})`: ""}
                  </span>
                )}
              </div>
